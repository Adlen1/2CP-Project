...
class _FindGamePageState extends State<FindGamePage> with TickerProviderStateMixin {
  ...
  void onButtonTap(int element) {
    setState(() {
      tappedElements.add(element); // Mark all buttons with this element as tapped
    });
  }
...
Widget _buildElementsContainer(List<String> elements, double width, double height) {
  return Container(
    width: width,
    height: height,
    ...
    child: Column(
      ...
      children: elements.map((elementPath) {
...
bool allButtonsTapped = tappedElements.length == (currentLevelData["buttons"] as List).map((b) => b["element"]).toSet().length;
  return Container(
    width: width*1.1,
    height: height,
    decoration: BoxDecoration(
            ...
            onTap: isTapped
                ...
                : () {
                    setState(() {
                      ...
                      _tappedButtons.add(element); // ✅ Track tapped elements

                      if (_tappedButtons.length == buttonData.map((b) => b["element"]).toSet().length) {
                        _stopTimer();
                      }
                    });
                  },
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                _buildElementsContainer(currentLevelData["elements"], containerWidth, containerHeight),

                // Right Container: Main Region Image + Hidden Buttons
                _buildImageContainer(currentLevelData["photo"], containerWidth2, containerHeight2),
              ],
            ),
          ),
        ),
